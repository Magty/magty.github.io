<template>
  <div>
    <div class="i-layout-page-header">
      <PageHeader title="Quill 编辑器" content="对 Quill 进行过封装，可以使用 v-model 绑定数据。" hidden-breadcrumb></PageHeader>
    </div>
    <Card class="ivu-mt" :bordered="false" dis-hover>
      <i-quill :min-height="400" v-model="value"></i-quill>
      <Divider></Divider>
      <Button @click="handleAddText">增加文本</Button>
      <Button @click="handleAddImg" class="ivu-ml">增加图片</Button>
    </Card>
    <Card class="ivu-mt" title="HTML 内容" :bordered="false" dis-hover>{{value}}</Card>
  </div>
</template>

<script>
import iQuill from './i-quill'
export default {
  name: 'editor-quill',
  components: {
    iQuill
