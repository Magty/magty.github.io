<template>
  <div>
    <div class="i-layout-page-header">
      <PageHeader
        title="simpleMDE 编辑器"
        content="对 simpleMDE 进行过封装，可以使用 v-model 绑定数据。"
        hidden-breadcrumb
      ></PageHeader>
    </div>
    <Card class="ivu-mt" :bordered="false" dis-hover>
      <i-mde ref="mde" :min-height="400" v-model="value"></i-mde>
      <Divider></Divider>
      <Button @click="handleAddText">增加文本</Button>
      <Button @click="handleAddImg" class="ivu-ml">增加图片</Button>
    </Card>
    <Card class="ivu-mt" title="Markdown 内容" :bordered="false" dis-hover>
      <pre>{{value}}</pre>
    </Card>
  </div>
</template>
