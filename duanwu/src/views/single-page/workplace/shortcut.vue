<template>
  <div class="dashboard-workplace-shortcut ivu-text-center">
    <Grid :col="3" :border="false" padding="0">
      <GridItem v-for="item in data" :key="item.name" class="ivu-mb">
        <i-link :to="item.link">
          <Icon v-if="item.icon" :type="item.icon" :color="item.color" size="24"></Icon>
          <Icon v-else-if="item.custom" :custom="item.custom" :color="item.color" size="24"></Icon>
          <p class="ivu-pt-8">{{item.name}}</p>
        </i-link>
      </GridItem>
    </Grid>
    <Divider class="ivu-mt-8 ivu-mb-8"></Divider>
    <Grid class="ivu-text-center" :col="4" :border="false" padding="0">
      <GridItem v-for="item in actions" :key="item.name" class="ivu-mt">
        <i-link :to="item.link">{{item.name}}</i-link>
      </GridItem>
      <GridItem class="ivu-mt">
        <Button @click="showAdd=true" icon="md-add" type="primary" ghost size="small">添加</Button>
      </GridItem>
    </Grid>
    <Modal @on-ok="handleAdd" v-model="showAdd" title="添加快捷操作" :loading="loading">
...
<style lang="scss">
.dashboard-workplace-shortcut a:hover {
  color: #2d8cf0;
}
</style>
