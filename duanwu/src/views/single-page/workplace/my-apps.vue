<template>
  <Grid class="dashboard-workplace-apps" :col="3" :border="false" padding="6px" hover>
    <GridItem v-for="(item,index) in data" :key="item.name">
      <div class="dashboard-workplace-apps-item" :style="`background-image:url(${item.cover})`">
        <div class="dashboard-workplace-apps-item-main">
          ...
          <div class="dashboard-workplace-apps-extra">
            ...
            <Tooltip v-if="item.collect" placement="top" content="取消星标">
              <Icon
                @click="handleUnCollect(index)"
                class="dashboard-workplace-apps-extra-star"
                type="md-star"
              ></Icon>
            </Tooltip>
            <Tooltip v-else placement="top" content="星标">
              <Icon @click="handleCollect(index)" type="md-star"></Icon>
            </Tooltip>
          </div>
        </div>
      </div>
...
.dashboard-workplace-apps {
  ...
  &-extra {
    ...
    i {
      margin-left: 4px;
      font-size: 16px;
      &.dashboard-workplace-apps-extra-star {
        color: #f90;
      }
      &:not(.dashboard-workplace-apps-extra-star) {
        display: none;
      }
    }
  }
  &-item:hover .dashboard-workplace-apps-extra i {
