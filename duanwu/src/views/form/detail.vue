<template>
  <div class="form-detail">
    <div class="i-layout-page-header">
      <PageHeader title="表单详情" content="表单除了提交数据，有时也用于显示只读信息。" hidden-breadcrumb></PageHeader>
    </div>
    <Card title="基本信息" class="ivu-mt" :bordered="false" dis-hover>
      <Form :label-width="labelWidth" :label-position="labelPosition">
        <Row :gutter="24">
          ...
          <i-Col v-bind="grid">
            <FormItem label="性别：">{{'male' === data.gender ? '男' : '女'}}</FormItem>
          </i-Col>
          <i-Col v-bind="grid">
            <FormItem label="城市：">{{data.city}}</FormItem>
          </i-Col>
          <i-Col v-bind="grid">
            <FormItem label="生日：">{{data.birthday}}</FormItem>
          </i-Col>
          <i-Col v-bind="grid">
            <FormItem label="邮箱：">{{data.mail}}</FormItem>
          </i-Col>
          <i-Col v-bind="grid">
    ...
    <Card title="其它信息" class="ivu-mt" :bordered="false" dis-hover>
      <Form :label-width="labelWidth" :label-position="labelPosition">
        <Row :gutter="24">
          <i-Col v-bind="grid2">
            <FormItem label="姓名：">{{data.name}}</FormItem>
    ...
    <Card title="表格信息" class="ivu-mt i-table-no-border" :bordered="false" dis-hover>
      <Table :columns="columns" :data="tableData">
        <template slot-scope="{row}" slot="name">
          <Avatar :src="row.avatar" size="small"></Avatar>
          <span class="ivu-ml-8">{{row.name}}</span>
        </template>
        <template slot-scope="{row}" slot="gender">{{'male' === row.gender ? '男' : '女'}}</template>
      </Table>
    </Card>
  </div>
</template>
