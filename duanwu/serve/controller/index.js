...
module.exports = {
  ...
  async getFile(req, res) {
    ...
    const fileInfo = await global.mysql.file_manage.findOne({
      where: {
        key
      }
    })
  ...
  async uploadFile(req, res) {
    ...
    const insertRes = await global.mysql.file_manage.create({
      path,
      file_size: size,
      file_type: type,
      file_name: name,
  ...
  async getFileList(req, res) {
    ...
    if (!userId) throw new Error('缺少必要参数')
    const fileList = await global.mysql.file_manage.findAll({
      where: {
        user: userId
      },
      attributes: {
  ...
  async deleteFile(req, res) {
    ...
    if (!key) throw new Error('缺少参数')
    const _res = global.mysql.file_manage.destroy({
      where: {
        key
      }
    })
